...
TEST_SUITE("ChebAnalytic") {
  TEST_CASE("double lengthEllipse" *
    ...
    const double rho = dis(gen);
    const double result = lengthEllipse_TEST(rho, n);
    // calculate length of ellipse using series expansion
    std::vector<long double> coeff{1.L / 4,      1.L / 64,     1.L / 256,
                                   25.L / 16384, 49.L / 65536, 441.L / 1048576};
    long double ref = 1.0;
    const long double h = 1.0L / (rho * rho * rho * rho);
    long double hp = h;
    ...
    ref *= M_PI * rho;
    CHECK(abs(result - ref) == doctest::Approx(0.).epsilon(1e-2));
  }

  TEST_CASE("std::pair<double, double> bestBound" *
            doctest::description(
                "calculate the approximation of $M = \\min_\\rho"
                " \\frac{\\vert\\gamma_\\rho\\vert \\max_{z \\in \\gamma_p}"
                " \\vert f(z) \\vert}{\\pi d([-1,1], \\gamma)}$")) {
    auto sol_M_rho = bestBound();
    const double sol_M = sol_M_rho.first;
    const double sol_rho = sol_M_rho.second;
    auto stud_M_rho = bestBound_TEST();
    const double stud_M = stud_M_rho.first;
    const double stud_rho = stud_M_rho.second;

    CHECK(abs(sol_M - stud_M) == doctest::Approx(0.).epsilon(eps));
    CHECK(abs(sol_rho - stud_rho) == doctest::Approx(0.).epsilon(eps));
  }
  ...
  TEST_CASE("std::pair<std::vector<double>, std::vector<double>> compareErrorAndBound" *
    ...
    Eigen::Map<Eigen::VectorXd> stud_upbd(stud_upbd_v.data(),
                                          stud_upbd_v.size());
    CHECK((sol_err - stud_err).norm() == doctest::Approx(0.).epsilon(eps));

    CHECK((sol_upbd - stud_upbd).norm() == doctest::Approx(0.).epsilon(eps));
  }
}
