...
int main() {
  ...
  if (ans == 1 || ans == 0) {
    Eigen::VectorXd Mesh(11);
    Mesh << 0.0, 1e-6, 1e-4, 0.01, 0.1, 0.2, 0.3, 0.45, 0.6, 0.8, 1.0;
    std::cout << "Testing pwlintpMaxError() on a non-uniform mesh M = { "
              << Mesh.transpose() << " }\n";
    double err_sqrt =
        pwlintpMaxError([](double x) { return std::sqrt(x); }, Mesh);
    double err_sin =
        pwlintpMaxError([](double x) { return std::sin(10 * x); }, Mesh);
  ...
  if (ans == 3 || ans == 0) {
    int n_alphas = 6;
    Eigen::VectorXd alpha = Eigen::VectorXd::LinSpaced(n_alphas, 0.25, 2.75);
    std::cout << "Testing cvgrateEquidistantMesh() for alpha = "
              << alpha.transpose() << " \n";
    Eigen::VectorXd ConvRates = cvgrateEquidistantMesh(alpha);
    std::cout << "Convergence rates: " << ConvRates.transpose() << "\n\n";
  }
  if (ans == 4 || ans == 0) {
  ...
  if (ans == 5 || ans == 0) {
    int n_alphas = 5;
    Eigen::VectorXd alpha = Eigen::VectorXd::LinSpaced(n_alphas, 0.25, 2.25);
    int n_betas = 4;
    Eigen::VectorXd beta = Eigen::VectorXd::LinSpaced(n_betas, 0.5, 2.0);
    std::cout << "Testing cvgrateGradedMesh() for\n\talpha = "
              << alpha.transpose() << ", and\n\tbeta = " << beta.transpose()
              << " \n";
    Eigen::MatrixXd ConvRatesGraded = cvgrateGradedMesh(alpha, beta);
