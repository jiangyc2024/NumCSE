...
/**
 * @brief The linear interpolant class.
 *
 */
/* SAM_LISTING_BEGIN_0 */
class LinearInterpolant {
 public:
   ...
   * @return double Value $I(x)$.
   */
  double operator()(double x);

 private:
  // TODO: (5-4.a) Put necessary members of the class here.
  // START
...
/* SAM_LISTING_BEGIN_1 */
LinearInterpolant::LinearInterpolant(const Eigen::VectorXd &t,
                                     const Eigen::VectorXd &y) {
  assert(t.size() == y.size() && "Input sizes must match");
  assert(t.size() > 1 && "Must specify at least two nodes");

  // TODO: (5-4.a) Implement the constructor of the class. t and y may not be
  // sorted.
  // START

  // helper lambda to transform two Eigen::VectorXd into one
  ...
  std::transform(t.data(), t.data() + t.size(), y.data(), i_points.begin(),
                 VecToVecPair);
...
double LinearInterpolant::operator()(double x) {
  double I_x = 0.;
  // TODO: (5-4.a) Implement an efficient evaluation operator.
  // START
  ...
  if ((it == i_points.begin() && it->first != x) || it == i_points.end())
    return 0;
