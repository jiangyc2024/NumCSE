#ifndef NATCUBSPLINE_HPP
#define NATCUBSPLINE_HPP
...
public:
  NaturalCubicSpline(Eigen::VectorXd t, Eigen::VectorXd y);
...
NaturalCubicSpline::NaturalCubicSpline(Eigen::VectorXd t, Eigen::VectorXd y)
...
bool NaturalCubicSpline::checkC2() const {
  const long n = t_.size() - 1;
  // Vector of h_i's , t has to be sorted
  auto h = (t_.tail(n) - t_.head(n)).array();
  // Vector h_i^2
  ...
  for (int j = 1; j <= n; ++j) {
    ...
    if (j > 1) {
      ...
      if (std::abs(spp_left) > 1.0E-10) {
        std::cout << "s''(t_0) != 0" << std::endl;
        error = true;
      }
    }
    spp = spp_right;
  }
  if (std::abs(spp) > 1.0E-10) {
    std::cout << "s''(t_0) != 0" << std::endl;
    error = true;
  }
...
int NaturalCubicSpline::getPtIdx(double t) const {
...
double NaturalCubicSpline::eval(double t) const {
