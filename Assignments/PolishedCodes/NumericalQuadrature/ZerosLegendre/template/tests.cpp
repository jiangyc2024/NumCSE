...
TEST_SUITE("ZerosLegendre") {
  ...
  TEST_CASE("double Pnx" * doctest::description("optional helper function") *
            doctest::skip()) {}

  TEST_CASE("Eigen::MatrixXd gaussPts" *
            doctest::description("gaussPts (8-10.d)")) {
    // This case will pass even though the function gives wrong output (in this
    // case for $P_8$). It just checks if the function is implemented like the
    // solution.
  ...
  TEST_CASE("Eigen::MatrixXd gaussPts_regulaFalsi" *
            doctest::description("gaussPts with regula falsi (8-10.f)")) {
    constexpr unsigned int n = 8;
