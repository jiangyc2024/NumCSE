...
void legvals(const Eigen::VectorXd& x, Eigen::MatrixXd& Lx, Eigen::MatrixXd& DLx) {
  ...
  // TODO: (8-10.c) Fill matrices Lx and DLx with values of the legendre polynomials at x, i.e. $\{P_k(x_j)\}_{jk}$ and $\{P'_k(x_j)\}_{jk}$
  // START
  assert(n >= 2);
...
double Pnx(const double x, const int n) {
  ...
  // TODO: (8-10.d, optional) Evaluate $P_n(x)$ for a scalar $x$ and integer $n$.
  // START
  assert(n >= 1);
  Px(0) = 1.; Px(1) = x;
  for (unsigned int k = 2; k <= n; ++k){
...
Eigen::MatrixXd gaussPts(const unsigned int n, const double rtol = 1e-10,
  ...
  // TODO: (8-10.d) Find the zeros of the legendre polynomials using the secant method.
  // START
...
/* SAM_LISTING_BEGIN_3 */
Eigen::MatrixXd gaussPts_regulaFalsi(const unsigned int n, const double rtol = 1e-10, const double atol = 1e-12) {
  Eigen::MatrixXd zeros = Eigen::MatrixXd::Zero(n, n);
  
  // TODO: (8-10.f) Find the zeros of the legendre polynomials using the secant method with regula falsi.
  // START
  double x0, x1, f0, f1, s;
  for (unsigned int k = 1; k <= n; ++k) {
    for (unsigned int j = 1; j <= k; ++j) {
