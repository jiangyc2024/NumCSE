...
void testConvGaussQuad() {
  ...
  auto integrand = [](double t) { return std::acos(t) * 1.0 / (1.0 + std::exp(t)); };

  // For recording errors etc.
  std::vector<cvgdata> data;
  // We suspect algebraic convergence. Accordingly use doubling of number of
  // quadrature nodes
  constexpr int L = 8;  // Use at most $2^L$ quadrature nodes
  int n = 2;
  for (int l = 1; l <= L; ++l, n *= 2) {
...
double arccosWeightedQuad(FUNCTION &&f, unsigned int n) {
  ...
  // a transformation of the quadrature nodes and weights, see \lref{rem:quadtrf}
