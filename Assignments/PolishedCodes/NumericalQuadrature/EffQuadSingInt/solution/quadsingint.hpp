...
//! @brief Compute integral $\int_{-1}^1 \sqrt(1-t^2) f(t) dt$ using transformation
...
double quadsingint(const Function& f, const unsigned n) {
  ...
  // TODO: (8-9.c) Compute $\int_{-1}^1 \sqrt(1-t^2) f(t) dt$ using Gauss-Legendre quadrature. Ensure asymptotic exponential convergence.
  // START
  // Method 1: $s = \sqrt(1 \pm t)$
  // Will use same node twice
  QuadRule Q = gauleg(n);
  
  // quadrature done on interval [0, 1], i.e. nodes and weights need to be shifted
  ...
  //
  // quadrature done on interval [-pi/2, pi/2], i.e. nodes and weights need to be shifted
...
//! @brief Tabulates and plots the quadrature error, i.e. $\abs{W(f) - \text{quadsingint(f,n)}}$ for
//! $f(t) = \frac{1}{2 + e^{3t}}$ and $n = 1,2,...,25$.
/* SAM_LISTING_BEGIN_2 */
