...
std::vector<double> fppchipConvergence() {
  ...
  for (unsigned int i = 4; i <= 512; i = i << 1) {
    ...
    Eigen::VectorXd s_zero_x = fppchip(f, t, x);

    // Compute infinity norm of error
    err_zero.push_back((s_zero_x - fx).lpNorm<Eigen::Infinity>());
    N_nodes.push_back(i);
...
std::vector<double> rspchipConververgence() {
  ...
  for (unsigned int i = 4; i <= 512; i = i << 1) {
    ...
    Eigen::VectorXd s_reconstr_x = s_reconstr.eval(x);

    // Compute infinity norm of error
    err_reconstr.push_back((s_reconstr_x - fx).lpNorm<Eigen::Infinity>());
    N_nodes.push_back(i);
