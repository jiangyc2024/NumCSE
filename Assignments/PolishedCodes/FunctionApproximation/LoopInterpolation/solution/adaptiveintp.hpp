...
std::vector<Eigen::Vector2d> closedPolygonalInterpolant(
  ...
  for (unsigned int i = 1; i <= n; ++i) {
    ...
    while (j < x.size() && x(j) * lambda[n] <= lambda[i]) {
      ...
      Eigen::Vector2d coord = (1 - ratio) * Sigma[i - 1] + ratio * Sigma[i];
...
std::vector<Eigen::Vector2d> closedHermiteInterpolant(
  ...
  Eigen::VectorXd x_scl = x * lambda[n];
  unsigned int j = 0;
  // loop over sides of polygon
  for (unsigned int i = 1; i <= n; ++i) {
    // loop over evaluation points x
...
void plotKite(const char *filename, double tol = 1.0e-3) {
  ...
  constexpr unsigned int nmin = 6;
  constexpr unsigned int N = 1e5;
  // initializing x, dsicarding the 1 at the end
  Eigen::VectorXd x = Eigen::VectorXd::LinSpaced(N + 1, 0, 1).head(N);

  std::pair<std::vector<Eigen::Vector2d>, std::vector<Eigen::Vector2d>> vec =
      adaptedHermiteInterpolant(c, nmin, x, tol);

  std::vector<Eigen::Vector2d> eval = vec.first;
  std::vector<Eigen::Vector2d> Sigma = vec.second;
  const unsigned int M = Sigma.size();
  Eigen::VectorXd x_hermite(N);
  Eigen::VectorXd y_hermite(N);
  Eigen::VectorXd x_poly(M);
  Eigen::VectorXd y_poly(M);
