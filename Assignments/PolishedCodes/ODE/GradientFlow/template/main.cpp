...
int main() {
   ...
   */
  MatrixXd Gsol(5,2);
  Gsol << 0.234048464188, 0.0911954850442,
          1.16124077723, 0.43498079856,
          0.589539233307, 0.228323917228,
          0.499164120689, 0.194780654616,
  ...
  for(int i = 0; i < 5; i++) {
    ...
    if ( G.at(i).size() == 2 && G.size() == 5 ) {
      ...
      if ((Gsol.row(i) - G.at(i).transpose()).norm() >= tol) {
        ...
        std::cout <<
                  "Test for computeStages() failed: wrong output.\n\n";
        break;
      }
    } else {
      passed_computeStages = false;
  ...
  if (passed_computeStages)
    std::cout << "Test for computeStages() passed!\n\n";


  /*
   *  run discEvolSDIRK()
   */
  VectorXd y1 = discEvolSDIRK(f,J,y0,h);
  std::cout << "y1 = discEvolSDIRK(f,J,y0,h) = " << y1.transpose() << std::endl;
  /*
   *  test discEvolSDIRK()
   */
  Vector2d y1sol(2.04447583358, 0.915950635186);
  tol = 1E-6;
  if (y1.size() == y1sol.size()) {
    if ( (y1 - y1sol).norm() < tol )
      ...
      std::cout
          << "Test for discEvolSDIRK() failed: wrong output.\n\n";
  } else
    std::cout
        << "Test for discEvolSDIRK() failed: wrong size.\n\n";
  ...
  MatrixXd Ysol(N+1,2);
  Ysol << 1, 0,
          0.661574350863, -0.265183334169,
          0.500522309998, -0.345715652452,
          0.415074659032, -0.354835307771,
          0.36129317252, -0.337895066294,
          0.321645925272, -0.312568326946,
          0.289168885473, -0.28564991388,
          0.26103327209, -0.259669864129,
          0.236018396524, -0.235490344277,
          0.213530281589, -0.21332581582,
          0.193223336322, -0.193144178833;
  // set to false as soon as an error is encountered:
  ...
  for(int i = 0; i <= N; i++) {
    ...
    if ( Y.at(i).size() == 2 && Y.size() == N+1 ) {
      ...
      if ((Ysol.row(i) - Y.at(i).transpose()).norm() >= tol) {
        ...
        std::cout <<
                  "Test for solveGradientFlow() failed: wrong output.\n\n";
        break;
      }
    } else {
      passed_solveGradientFlow = false;
