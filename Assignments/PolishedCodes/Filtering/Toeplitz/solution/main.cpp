...
int main() {
  ...
  Eigen::VectorXd y_1 = toepmatmult(c, r, x);
  Eigen::VectorXd y_2 = toepmult(c, r, x);
  std::cout << "Error = " << (y_1 - y_2).norm() << std::endl;

  // Initialization
  n = 4;
  Eigen::VectorXd h(n), y(n);
  h << 1, 2, 3, 4;
  y << 5, 6, 7, 8;

  // Compute with both functions ttmatsolve and ttrecsolve
  std::cout << "Check that ttmatsolve and ttrecsolve are correct" << std::endl;
  Eigen::VectorXd x_1 = ttmatsolve(h, y);
  Eigen::VectorXd x_2 = ttrecsolve(h, y, 2);
