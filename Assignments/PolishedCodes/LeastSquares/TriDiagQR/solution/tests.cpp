...
TEST_SUITE("TriDiagQR") {
  TEST_CASE("inline double sign" * doctest::description("skipped") *
            doctest::skip()) {}

  TEST_CASE("int sgn" * doctest::description("skipped") * doctest::skip()) {}

  TEST_CASE("inline Eigen::Matrix2d Givens" * doctest::description("skipped") *
            doctest::skip()) {}

  TEST_CASE("std::tuple<double, double, double> compGivensRotation" *
            doctest::description("Givens rotation")) {
    ...
    for (Eigen::Index k = 0; k < a.cols(); ++k) {
      ...
      CHECK(std::get<0>(params_sol) ==
            doctest::Approx(std::get<0>(params_stud)).epsilon(eps));
      CHECK(std::get<1>(params_sol) ==
            doctest::Approx(std::get<1>(params_stud)).epsilon(eps));
      CHECK(std::get<2>(params_sol) ==
            doctest::Approx(std::get<2>(params_stud)).epsilon(eps));
    }
  }

  TEST_CASE("TriDiagonalQR [OUT OF CLASS]" *
            doctest::description("Constructor")) {
    ...
    CHECK((Q_sol - Q_stud).norm() == doctest::Approx(0.).epsilon(eps));
    CHECK((R_sol - R_stud).norm() == doctest::Approx(0.).epsilon(eps));
  ...
  TEST_CASE("Eigen::VectorXd applyQT [OUT OF CLASS]" *
            doctest::description("applyQT()")) {
    TriDiagonalMatrix A(data.d_A, data.l_A, data.u_A);
    TriDiagonalQR sol_A(A);
    TriDiagonalQR_TEST stud_A(A);
  ...
  TEST_CASE("Eigen::VectorXd solve [OUT OF CLASS]" *
            doctest::description("solve()")) {
    TriDiagonalMatrix A(data.d_A, data.l_A, data.u_A);
    TriDiagonalQR sol_A(A);
    TriDiagonalQR_TEST stud_A(A);
    ...
    SUBCASE("normal test") {
      ...
      //			Eigen::VectorXd spaced =
      // Eigen::VectorXd::LinSpaced(10, 1, 10);
      // Eigen::MatrixXd A =
      // spaced.asDiagonal(); 			A.diagonal<-1>() =
      // Eigen::VectorXd::LinSpaced(9, 3, 12);
      // A.diagonal<1>() = spaced.head(9); Eigen::VectorXd x =
      // A.lu().solve(spaced); 			TriDiagonalMatrix
      // spaced_tri(spaced, Eigen::VectorXd::LinSpaced(9, 3, 12),
      // spaced.head(9)); 			TriDiagonalQR
      // spaced_QR(spaced_tri); 			Eigen::VectorXd x_spaced
      // =
      // spaced_QR.solve(spaced); 			CHECK((x -
      // x_spaced).norm() == doctest::Approx(0.).epsilon(1e-9));
      //
      //			Eigen::VectorXd random =
      // Eigen::VectorXd::Random(10); 			Eigen::MatrixXd B =
      // random.asDiagonal(); 			Eigen::VectorXd random2 =
      // Eigen::VectorXd::Random(9); 			B.diagonal<-1>() =
      // random2; 			B.diagonal<1>() = spaced.head(9);
      // x = B.lu().solve(random); 			TriDiagonalMatrix
      // random_tri(random, random2, spaced.head(9));
      // TriDiagonalQR
      // random_QR(random_tri); 			Eigen::VectorXd x_random
      // =
      // random_QR.solve(random); 			CHECK((x -
      // x_random).norm() == doctest::Approx(0.).epsilon(1e-9));
  ...
  TEST_CASE(
      "std::pair<Eigen::MatrixXd,Eigen::MatrixXd> getQRFactors [OUT OF CLASS]" *
      doctest::description("skipped") * doctest::skip()) {}

  TEST_CASE("unsigned int invit" * doctest::description("skipped") *
            doctest::skip()) {}

  TEST_CASE("unsigned int invit<TriDiagonalMatrix>" *
            doctest::description("invit()")) {
    TriDiagonalMatrix A(data.d_A, data.l_A, data.u_A);
    TriDiagonalQR sol_A(A);
    TriDiagonalQR_TEST stud_A(A);
    ...
    invit<TriDiagonalMatrix>(A, x_sol, 1E-10, 100);
    invit_TEST<TriDiagonalMatrix>(A, x_stud, 1E-10, 100);
