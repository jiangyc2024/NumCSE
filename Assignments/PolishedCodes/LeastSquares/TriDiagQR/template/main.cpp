...
int main() {
  ...
  if (ans == 1 || ans == 0) {
    ...
    a << 0, 4, -2, 4, -1.22, 4, 0, 2, -3, -2.85;
    std::tuple<double, double, double> params;
    Eigen::Matrix2d G, Grho;
    ...
    for (int k = 0; k < a.cols(); k++) {
      ...
      std::cout << "Rotation G=\n" << G << std::endl;
      std::cout << "Rotated vector (G^T)*a = (" << a.col(k).transpose() * G
                << ")^T\n";
      std::cout << "(rho, gamma, sigma) = (" << rho << ", " << gamma << ", "
                << sigma << ")\n";
      std::cout << "Rotation constructed from rho agrees with G: "
                << ((G - Grho).norm() < 1E-8) << "\n\n";
    }
    if (ans != 0) return 0;
  }
  int n = 6;
  Eigen::VectorXd d(n), l(n - 1), u(n - 1);
  l = Eigen::VectorXd::LinSpaced(n - 1, 1, n - 1);
  d = Eigen::VectorXd::LinSpaced(n, n, 2 * n - 1);
  u = Eigen::VectorXd::LinSpaced(n - 1, 2 * n, 3 * n - 2);
  ...
  if (ans == 3 || ans == 0) {
    ...
    x = Eigen::VectorXd::LinSpaced(n, 1, n);
    y = Aqr.applyQT(x);
    z = Q.transpose() * x;
  ...
  if (ans == 4 || ans == 0) {
    ...
    z = Eigen::VectorXd::LinSpaced(n, 1, n);
    ...
    std::cout
        << "Using a non-invertible matrix should throw a runtime error.\n";
    B.d(0) = 0.;
    B.u(0) = 0.;
    Bqr = TriDiagonalQR(B);
    try {
  ...
  if (ans == 5 || ans == 0) {
    ...
    M = invit<Eigen::MatrixXd>(Adense, y);
    ...
    std::cout << "N = " << N << std::endl;
    M = invit<Eigen::MatrixXd>(Adense, y, 1E-10, 100);
    std::cout << "Difference from dense implementation: " << (N - M) << "\n\n";
