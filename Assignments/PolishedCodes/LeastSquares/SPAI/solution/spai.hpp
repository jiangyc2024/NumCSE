...
Eigen::SparseMatrix<double> spai(Eigen::SparseMatrix<double>& A) {
  ...
  for (unsigned int i = 0; i < N; ++i) {
    ...
    // onto $\mathcal{P}(a_i)$
    Eigen::SparseMatrix<double> C(N, nnz_i);
    ...
    for (unsigned int k = outPtr[i]; k < outPtr[i + 1]; ++k) {
      ...
      for (unsigned int l = 0; l < nnz_k; ++l) {
        ...
        C_triplets.emplace_back(Eigen::Triplet<double>(
            innPtr[innIdx], k - outPtr[i], valPtr[innIdx]));
      }
    }
    ...
    Eigen::SparseMatrix<double> S = C.transpose() * C;
    Eigen::MatrixXd M = Eigen::MatrixXd(S);
    Eigen::VectorXd xt = C.row(i).transpose();
    Eigen::VectorXd b = M.partialPivLu().solve(xt);

    // Loop of length at most $n$.
    for (unsigned int k = 0; k < b.size(); ++k) {
      triplets.emplace_back(
          Eigen::Triplet<double>(innPtr[outPtr[i] + k], i, b(k)));
    }
  }
