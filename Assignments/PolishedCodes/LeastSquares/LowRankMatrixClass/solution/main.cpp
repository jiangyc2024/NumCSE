...
int main() {
  ...
  Eigen::MatrixXd A(4, 2), B(3, 2), C(3, 2), D(4, 2), E(3, 1), F(4, 1), G(3, 4);
  A << 1, 2, 3, 4, 5, 6, 7, 8;
  B << 9, 0, 1, 2, 3, 4;
  C << 5, 6, 7, 8, 9, 0;
  ...
  if (MC.cols() == D.cols() && MC.rows() == D.rows()) {
    if ((MC - D).norm() < tol)
      ...
      std::cout << "Test for operator*() failed: wrong output.\n\n";
  } else
    std::cout << "Test for operator*() failed: wrong size.\n\n";
  ...
  M *= C;
  Eigen::MatrixXd Me =
      M * Eigen::MatrixXd::Identity(M.cols(), M.cols());  // Eigen version of M.
  ...
  if (Me.cols() == D.cols() && Me.rows() == D.rows()) {
    if ((Me - D).norm() < tol && M.cols() == D.cols() && M.rows() == D.rows())
      ...
      std::cout << "Test for operator*=() failed.\n\n";
  } else
    std::cout << "Test for operator*=() failed: wrong size.\n\n";
  ...
  N.addTo(L);
  Eigen::MatrixXd Ne =
      N * Eigen::MatrixXd::Identity(N.cols(), N.cols());  // Eigen version of N
  ...
  if (Ne.cols() == G.cols() && Ne.rows() == G.rows()) {
    if ((Ne - G).norm() < tol && N.rank() == svd.rank() &&
        N.cols() == G.cols() && N.rows() == G.rows())
      std::cout << "Test for addTo() passed!\n\n";
    else
      std::cout << "Test for addTo() failed.\n\n";
  } else
    std::cout << "Test for addTo() failed: wrong size.\n\n";
