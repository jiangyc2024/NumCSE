...
struct Topology {
   ...
   *
   * @param V Source voltage $V$ at node $16$ (in Volt), ground is set to $0V$
   * at node $17$
   */
  Topology(double V) {
...
// END
{
  ...
  Eigen::MatrixXd A_Rx = Eigen::MatrixXd::Zero(15, 15);
...
/**
 * @brief Access operator for the values of the potential at the nodes
 *
 * @param V Source voltage $V$ at node $16$ (in Volt)
 * @return VectorXd of potentials at nodes
 */
/* SAM_LISTING_BEGIN_2 */
 ...
 *
 * @param R Resistance (in Ohm) value of $R$
 * @param V Source voltage $V$ at node $16$ (in Volt), ground is set to $0V$ at
 * node $17$
 */
/* SAM_LISTING_BEGIN_4 */
...
// END
{
  ...
  Eigen::MatrixXd A0 = Eigen::MatrixXd::Zero(nnodes_, nnodes_);
  for (resistor& res : top.T) {
    // Shift indices down by 1 (indices in Figure start from 1)
    std::size_t i = res.first - 1;
    std::size_t j = res.second - 1;
  ...
  // ($R$ is the  resistance between node $i$ and ground/source
  // node, $V$ is voltage at sink or source)
  // and to its own diagonal with $R$
  Eigen::VectorXd b = Eigen::VectorXd::Zero(nnodes_);
  for (voltage& volt : top.S) {
    // Shift index down by 1 and get voltage in V2
    int i = std::get<0>(volt) - 1;
  ...
  Eigen::PartialPivLU<Eigen::MatrixXd> lu(A0);
  w_ = lu.solve(b);
  Eigen::VectorXd u0 = Eigen::VectorXd::Zero(nnodes_);
  u0(13) = -1.;
  u0(14) = 1.;
  z_ = lu.solve(u0);
  alpha_ = u0.dot(z_);
 ...
 * @return double Impedance $V / I$ of the system $A_{R_x}$
 */
/* SAM_LISTING_BEGIN_5 */
double ImpedanceMap::operator()(double Rx) const {
  ...
  Eigen::VectorXd x = w_ - xi * beta_ / (1. + xi * alpha_) * z_;
