...
TEST_SUITE("Arrow matrix x vector multiplication") {
  TEST_CASE("void arrow_matrix_2_times_x" *
            doctest::description("already implemented") * doctest::skip()) {}

  TEST_CASE("void efficient_arrow_matrix_2_times_x" *
      doctest::description("efficient arrow matrix x vector multiplication")) {
    ...
    CHECK((y_sol - y_stud).norm() == doctest::Approx(0.).epsilon(eps));
    ...
    CHECK((y_stud_ident - a).norm() == doctest::Approx(0.).epsilon(eps));
    ...
    CHECK((y_sol2 - y_stud2).norm() == doctest::Approx(0.).epsilon(eps));
  }

  TEST_CASE("void runtime_arrow_matrix" *
            doctest::description("Test and compare runtime")) {
    MESSAGE("This function wasn't tested. Run the program to see its output.");
  }

  TEST_CASE("duration_t timing" *
            doctest::description("implemented only in solution") *
            doctest::skip()) {}

  TEST_CASE("void runtime_arrow_matrix_with_chrono" *
            doctest::description("implemented only in solution") *
            doctest::skip()) {}
}
