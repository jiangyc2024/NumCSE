...
void tripletToCRS(const TripletMatrix<scalar>& T, CRSMatrix<scalar>& C) {
  ...
  for(auto triplet_it = T.triplets.begin(); triplet_it != T.triplets.end(); ++triplet_it) {
    ...
    auto lb_it = std::lower_bound(row.begin(), row.end(), col_val); // returns an iterator to a ColValPair in row
    ...
    if(lb_it != row.end() && lb_it->col == triplet_it->j) {
			// If we wouldnt declare the variable row as reference we wouldnt change the actual entries of the matrix C
