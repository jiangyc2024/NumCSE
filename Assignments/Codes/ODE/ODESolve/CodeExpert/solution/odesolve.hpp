...
//! Vector&) \param[in] p parameter p for construction of Psi tilde \param[in] h
//! step-size \param[in] y0 previous step
/* SAM_LISTING_BEGIN_0 */
...
//! \tparam DiscEvlOp type for evolution operator (e.g. lambda function type)
//! \param[in] Psi original evolution operator, must have operator(double, const
//! Vector&) \param[in] T final time \param[in] y0 initial data \param[in] N
//! number of steps
/* SAM_LISTING_BEGIN_1 */
template <class DiscEvlOp>
std::vector<Vector> odeintequi(DiscEvlOp &&Psi, double T, const Vector &y0,
                               unsigned int N) {
  std::vector<Vector> Y;
...
//! const Vector&) \param[in] T final time \param[in] y0 initial data \param[in]
//! h0 initial step size \param[in] p parameter p for construction of Psi tilde
