...
TEST_SUITE("InitCondLV") {
	TEST_CASE("std::pair<Vector2d, Matrix2d> PhiAndW" * doctest::description("Test of PhiAndW")) {
		std::pair<Eigen::Vector2d, Eigen::Matrix2d> PaW_sol = PhiAndW(data.u0, data.v0, data.T);
		std::pair<Eigen::Vector2d, Eigen::Matrix2d> PaW_stud = PhiAndW_TEST(data.u0, data.v0, data.T);
	...
	TEST_CASE("Vector2d findInitCond" * doctest::description("findInitCond: y")) {
