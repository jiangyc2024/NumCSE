...
computeIsolinePoints(GradientFunctor &&gradF, Eigen::Vector2d y0, double T) {
  ...
  auto rhs = [gradF](const Vector2d &x) {
    Vector2d gradFx = gradF(x);
    Vector2d out(-gradFx(1), gradFx(0)); // rotate
    out /= gradFx.norm();                // scale
    return out;
  };
  // Adaptive explicit Runge-Kutta method
  ode45<Vector2d> integrator(rhs);
  ...
  std::vector<std::pair<Vector2d, double>> sol = integrator.solve(y0, T);
  // Convert output into requested format: points on isoline arranged into the
  // columns of a matrix.
  int N = sol.size() - 1;
  States = MatrixXd::Zero(2, N + 1);
  for (int i = 0; i <= N; i++) {
...
Eigen::Matrix<double, 2, Eigen::Dynamic> crookedEgg() {
  ...
  Vector2d y0(1., 0.);
  double T = 4.;
  auto gradF = [](const Vector2d &x) {
    double x2 = x(0) * x(0), y2 = x(1) * x(1);
    double x2y2 = 4. * (x2 + y2);
    Vector2d grad(x2y2 * x(0) - 3. * x2, x2y2 * x(1) - 3. * y2);
    return grad;
  };
...
computeIsolinePointsDQ(FFunctor &&F, Eigen::Vector2d y0, double T) {
  ...
  auto gradF = [epsrt, F](const Vector2d &x) {
    Vector2d grad, dx, dy;
    dx << epsrt, 0.;
    dy << 0., epsrt;
