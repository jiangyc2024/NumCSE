...
int main() {
  ...
  std::cout << "Output of computeIsolinePoints() is a " << isoline.rows() << " by " << isoline.cols() << " matrix";
  if( isoline.cols() > 5 ) std::cout << ", starting with\n" << isoline.leftCols(5) << " ...\n";
  else std::cout << ":\n" << isoline << std::endl;
  ...
  if ( isoline.rows() == 2 && isoline.cols() >= 10 ) {
    ...
    if ( passed_computeIsolinePoints )
      ...
      std::cout
          << "Test for computeIsolinePoints() failed.\n\n";
  } else
    std::cout
        << "Test for computeIsolinePoints() failed: wrong size.\n\n";
  
  /*
   *  run crookedEgg()
   */
  MatrixXd crookedEggCurve = crookedEgg();
  std::cout << "Output of crookedEgg() is a " << crookedEggCurve.rows() << " by " << crookedEggCurve.cols() << " matrix";
  if( crookedEggCurve.cols() > 5 ) std::cout << ", starting with\n" << crookedEggCurve.leftCols(5) << " ...\n";
  ...
  if ( crookedEggCurve.rows() == 2 && crookedEggCurve.cols() >= 10 ) {
    ...
    for( int i = 1; i < crookedEggCurve.cols() ; i++ ) {
      ...
      // Check that point is not stationary:
      double step = (crookedEggCurve.col(i) - crookedEggCurve.col(i-1)).norm();
      if( step < tol*tol || err > tol ) {
    ...
    if ( passed_crookedEgg )
      ...
      std::cout
          << "Test for crookedEgg() failed.\n\n";
  } else
    std::cout
        << "Test for crookedEgg() failed: wrong size.\n\n";

  /*
   *  run computeIsolinePointsDQ()
   */
  MatrixXd isolineDQ = computeIsolinePointsDQ(G,y0,5.);
  std::cout << "Output of computeIsolinePointsDQ() is a " << isolineDQ.rows() << " by " << isolineDQ.cols() << " matrix";
  if( isolineDQ.cols() > 5 ) std::cout << ", starting with\n" << isolineDQ.leftCols(5) << " ...\n";
  else std::cout << ":\n" << isolineDQ << std::endl;
  ...
  if ( isolineDQ.rows() == 2 && isolineDQ.cols() >= 10 ) {
    ...
    if ( passed_computeIsolinePointsDQ )
      ...
      std::cout
          << "Test for computeIsolinePointsDQ() failed.\n\n";
  } else
    std::cout
        << "Test for computeIsolinePointsDQ() failed: wrong size.\n\n";
