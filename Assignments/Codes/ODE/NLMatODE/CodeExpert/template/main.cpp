...
int main() {
    ...
    bool is_invariant = checkinvariant(Y0, T);
    bool is_invar_sym = checkinvariant(M0, T);
    std::cout << "Test whether invariant was preserved or not..." << std::endl;
    
    if( is_invariant ) {
        std::cout << "Invariant for Y0 preserved."
                  << std::endl;
    } else {
        std::cout << "Invariant for Y0 NOT preserved."
                  << std::endl;
    }
    if( is_invar_sym ) {
        std::cout << "Invariant for M0 preserved."
                  << std::endl;
    } else {
