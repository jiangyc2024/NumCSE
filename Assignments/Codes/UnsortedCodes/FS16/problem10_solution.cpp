...
void notknotsplines (const Eigen::VectorXd &t, const Eigen::VectorXd &y, Eigen::VectorXd &c) {
    ...
    // System matrix and rhs as in 3.5.9, adding the two remaining conditions
    Eigen::SparseMatrix<double> A(n+1,n+1);
    Eigen::VectorXd b(n+1);
    ...
    // Reuse computation for rhs
    double bold = (y(1) - y(0)) / (h(0)*h(0));
    b(0) = 2*((y(1)-y(0))/(h(0)*h(0)*h(0))+(y(1)-y(2))/(h(1)*h(1)*h(1)));
    // Fill matrix A and rhs b
    for(int i = 1; i < n; ++i) {
        // Fill in a
        A.coeffRef(i,i-1) = h(i);
    ...
    b(n) = 2*((y(n-1)-y(n-2))/(h(n-2)*h(n-2)*h(n-2))+(y(n-1)-y(n))/(h(n-1)*h(n-1)*h(n-1)));
    // Compress the matrix
    A.makeCompressed();
    std::cout << A;
