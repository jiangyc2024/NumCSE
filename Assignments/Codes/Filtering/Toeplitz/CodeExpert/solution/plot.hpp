 ...
 * in loglog-plot
*/
void plot( std::vector<double> &vec_size, std::vector<double> &elap_time1, std::vector<double> &elap_time2, const std::string &fig_name, const std::string &label1, const std::string &label2){
  ...
  for( unsigned int i = 0; i<n; i++){
    vec_size_lin[i]  = vec_size[i] / vec_size[0] * elap_time1[0];
    vec_size_pow3[i] = pow(vec_size[i], 3) / pow(vec_size[0], 3) * elap_time2[0];
  }

  //make sure the sizes match
  assert( vec_size.size() == elap_time1.size() && "vector sizes must be the same.");
  ...
  plt::loglog(vec_size, vec_size_pow3, "--",{{"label", "0(n³)"}, {"color", "grey"}} );
  plt::legend("best");
