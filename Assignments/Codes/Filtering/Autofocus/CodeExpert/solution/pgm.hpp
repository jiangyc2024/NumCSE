...
class PGMObject {
 public:
  ...
  PGMObject(const Eigen::MatrixXd &data) { set_data(data); }
  ...
  PGMObject &set_data(const Eigen::MatrixXd &mat) {
    ...
    if (M > _upperBoundVal) {
      std::cerr << "Maximum value to high!" << std::endl;
      return *this;
    }
    ...
    if (is_long) {
      if (sizeof(unsigned short int) != 2) {
        std::cerr << "Internal error, invalid "
                  << "size of 'short unsigned int'." << std::endl;
        return *this;
      }
      mat = Eigen::Map<Eigen::Matrix<unsigned short int, Eigen::Dynamic,
                                     Eigen::Dynamic, Eigen::RowMajor> >(
                (unsigned short int *)data, height, width)
                .cast<double>();
    } else {
      mat = Eigen::Map<Eigen::Matrix<unsigned char, Eigen::Dynamic,
                                     Eigen::Dynamic, Eigen::RowMajor> >(
                data, height, width)
                .cast<double>();
    }
  ...
  friend std::istream &operator>>(std::istream &i, PGMObject &obj) {
    if (!i.good()) {
      std::cerr << "Input stream in bad state (missing file?)!" << std::endl;
      return i;
    }
    unsigned int stage = 0;
    unsigned int line = 1;
    while (stage != 3 || i.peek() == std::char_traits<char>::to_int_type('#')) {
      if (i.peek() != std::char_traits<char>::to_int_type('#')) {
        std::string mn;
        switch (stage) {
          case 0:
            //                    std::cout << "Reading magic number on line = "
            //                    << line
            //                              << "."
            //                              << std::endl;
            if (!std::getline(i, mn)) {
              std::cerr << "Bad, missing or corrupt "
                        << "file at line '" << mn << "' in input stream!"
                        << std::endl;
            }
            if (obj._magic_number != mn) {
              std::cerr << "Bad magic number '" << mn << "' in input stream!"
                        << std::endl;
              return i;
            }
            break;
          case 1:
            //                    std::cout << "Reading image size on line = "
            //                    << line
            //                              << "."
            //                              << std::endl;
            i >> obj.width >> obj.height;
            break;
          case 2:
            //                    std::cout << "Reading max val on line = " <<
            //                    line
            //                              << "."
            //                              << std::endl;
            i >> obj.maxVal;
            if (obj.maxVal > obj._upperBoundVal) {
              std::cerr << "Invalid max val!" << std::endl;
              return i;
            }
            if (obj.maxVal >= 256) {
              obj.is_long = true;
  ...
  friend std::ostream &operator<<(std::ostream &o, const PGMObject &obj) {
    if (!o.good()) {
      std::cerr << "Output in bad state (cannot write to disk?)!" << std::endl;
      return o;
    }
