 ...
 * $\gamma_p(\theta):=cos(\theta-i \log(\rho),\forall 0\leq\theta\leq2\pi,\rho>1$
 * by sampling for N equidistant arguments \theta
...
double lengthEllipse(double rho, unsigned int N) {
  // TODO: (7-5.a) Approximate the length of ellipse given $\rho$ and N equidistant points (in $\theta$)
  double length = 0.0;
  // START
  auto f = [rho](long double theta) { return 0.5*sqrt(rho*rho+1/(rho*rho)-2*cos(2*theta));};
...
* $\min_\rho \frac{\vert\gamma_\rho\vert \max_{z \in \gamma_p} \vert f(z) \vert}{\pi d([-1,1], \gamma)}$
* and the $\rho$ for which the minimum is attained
*/
/* SAM_LISTING_BEGIN_1 */
std::pair<double, double> bestBound() {
  // TODO: (7-5.d) Compute (7.5.2) with d from (7.5.3) and $\vert\gamma_\rho\vert$ from (7-5.a)
  // START
  ...
  VectorXd rhos = rhosClosedIntervall.segment(1, N - 2);
  ...
  for (int k = 0; k < rhos.size(); ++k) {
    ...
    const double M = lengthEllipse(rho, N)*fz.cwiseAbs().maxCoeff()*M_1_PI; // nominator of expression
 ...
 * \return the pair of the list of calculated error and the list of estimated upperbound
 */
/* SAM_LISTING_BEGIN_2 */
std::pair<std::vector<double>,std::vector<double>> compareErrorAndBound(unsigned int n_max) {
  // TODO: (7-5.f)
  // BEGIN
