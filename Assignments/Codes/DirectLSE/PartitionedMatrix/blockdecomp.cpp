...
void solvelse(const Matrix & R, const Vector & v, const Vector & u,
    ...
    // Tell eigen R is triangular, we use the construct
    // R.template to avoid compilation errors
...
int main() {
    ...
    for(unsigned int p = 2; p <= 10; p++) {
        ...
        std::cout << "Own implementation took:" << std::endl
                  << tm_own.min() / 1000000. << " ms" << std::endl;
        std::cout << "Eigen solver took:" << std::endl
                  << tm_eigen_lu.min() / 1000000. << " ms" << std::endl;
    }
}
