...
Scalar mod_newt_step_scalar(const Scalar& x,
  ...
  // TODO: (9-9.b) Compute a step of the modified Newton method for a scalar function
  // START
  Scalar y = x + f(x) / df(x);
...
bool sample_nonlinear_solver(const StepFunction& step,
  ...
  // TODO: (9-9.c,f) Implement this generic function that iteratively solves a nonlinear equation. The function should return whether the iteration converged in the maximum number of iterations.
  // START
...
void mod_newt_ord(void) {
  ...
  auto f = [] (double x) { return atan(x) - a; }; // Function which sees a as it is constexpr
  double x_scalar = 5; // initial guess
  
  // TODO: (9-9.c) Generate suitable terminal output to determine the order of convergence of the modified Newton method applied to the scalar equation that is given by $f = 0$. You may want to implement the templated function sample\_nonlinear\_solver for the iterations.
  // START
...
Vector mod_newt_step_system(const Vector &x,
  ...
  // TODO: (9-9.d) Efficiently perform one step of the modified Newton method for a system of equations.
  // START
  auto lu = df(x).lu();
 ...
 */
/* SAM_LISTING_BEGIN_5 */
Eigen::VectorXd mod_newt_sys(const Eigen::MatrixXd &A, const Eigen::VectorXd &c, double tol = 1.0e-6, int maxit = 100) {
  Eigen::VectorXd x_system = Eigen::VectorXd::Zero(c.size()); // Initial guess
  
  // TODO: (9-9.f) Solve the non-linear system of equations (9.9.6) using the modified Newton method. You may want to use your implementation of sample\_nonlinear\_solver.
  // START
  ...
  // Refactor (i.e.\ wrap) the step such that it is compatible with sample\_nonlinear\_solver.
  // Must see $F$, $dF$ and 'err'.
  // We also store the error.
  auto newt_system_step = [&F, &dF, &err] (const Eigen::VectorXd& x) -> Eigen::VectorXd {
...
void mod_newt_sys_test(void) {
  ...
  // TODO: (9-9.g) Determine the order of convergence from suitable tabulated values.
  // START
  // determine a reference solution with very low tolerance (machine precision)
  ...
  // seperate calculation and printing to not pollute the table from output of iteration function
  // increase the number of maximal iterations
  ...
  for (unsigned k = 0; k < 5; ++k) {
    ...
    if(k >= 1 && k < 4) {
      std::cout << space << (log(err.at(k + 1)) - log(err.at(k))) / (log(err.at(k)) - log(err.at(k - 1)));
    }
    std::cout << std::endl;
  }
  // END
