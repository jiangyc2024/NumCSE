...
TEST_SUITE("Modified Newton method") {
  TEST_CASE("double norm" * doctest::description("some helpers") *
            doctest::skip()) {}

  TEST_CASE("Scalar mod_newt_step_scalar" *
            doctest::description("scalar modified Newton step")) {
  ...
  TEST_CASE("bool sample_nonlinear_solver" *
            doctest::description("nonlinear solver")) {
    auto f = data.f;
    auto df = data.df;
    ...
    auto errf = [x_ex](double& x) { return std::abs(x - x_ex); };
  ...
  TEST_CASE("void mod_newt_ord" *
            doctest::description("scalar convergence order")) {
    MESSAGE("This function wasn't tested. Run the program to see its output.");
  }

  TEST_CASE("Vector mod_newt_step_system" *
            doctest::description("system modified Newton step")) {
    auto A = data.A;
    auto c = data.c;
  ...
  TEST_CASE("Eigen::VectorXd mod_newt_sys" *
            doctest::description("system iteration solver")) {
    const Eigen::VectorXd sol = mod_newt_sys(data.A, data.c);
    const Eigen::VectorXd stud = mod_newt_sys_TEST(data.A, data.c);
    CHECK((sol - stud).norm() == doctest::Approx(0.).epsilon(1e-9));
  }

  TEST_CASE("void mod_newt_sys_test" *
            doctest::description("system convergence order")) {
    MESSAGE("This function wasn't tested. Run the program to see its output.");
  }
}
