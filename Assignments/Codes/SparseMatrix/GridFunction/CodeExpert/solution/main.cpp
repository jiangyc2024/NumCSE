...
int main() {
    ...
    index_t n_small = 3, m_small = 3;
    std::cout << "--> Building matrix A..."
              << std::endl;
              
    SparseMatrix<double> A_small= build_matrix(S, shape_t{n_small, m_small});
    std::cout << MatrixXd(A_small) << std::endl;
    ...
    std::cout << "--> Solving A*vec(X) = vec(Y)..."
              << std::endl;
    solve(A, Y, X);
    std::cout << "--> The Frobenius norm of the matrix X is "
              << X.norm() << std::endl;
}
