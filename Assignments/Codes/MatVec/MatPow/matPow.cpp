...
#if INTERNAL
    mgl::Figure fig;
    fig.title("Timings of matPow");
    fig.ranges(2, std::pow(2, 31), 1e-6, 0.2e-3);
    fig.setlog(true, false);
    fig.plot(powers, times_matPow, " r+").label("own implementation");
    fig.plot(powers, times_Eigen_pow, " b+").label("Eigen");
    fig.xlabel("Power (k)");
    fig.ylabel("Time [s]");
    fig.legend(0, 1);
    fig.save("matpow_timing.eps");
    fig.save("matpow_timing.png");
#endif // INTERNAL
