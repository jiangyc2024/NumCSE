 ...
 */
/* SAM_LISTING_BEGIN_1 */
void test_kron(const MatrixXd &A, const MatrixXd &B, MatrixXd &C) {  // Allocate enough space for the matrix
 ...
 */
/* SAM_LISTING_BEGIN_2 */
void test_kron_mult(const MatrixXd &A, const MatrixXd &B, const VectorXd &x,               VectorXd &y) {
 ...
 */
/* SAM_LISTING_BEGIN_3 */
void test_kron_reshape(const MatrixXd &A, const MatrixXd &B, const VectorXd &x,                  VectorXd &y) {
  assert(A.rows() == A.cols() && A.rows() == B.rows() && B.rows() == B.cols() &&
         "Matrices A and B must be square matrices with same size!");
  unsigned int n = A.rows();
