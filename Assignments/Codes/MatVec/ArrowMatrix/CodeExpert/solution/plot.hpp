 ...
 * in loglog-plot
*/
void plot( std::vector<double> &vec_size, std::vector<double> &elap_time, std::vector<double> &elap_time_eff, const std::string &fig_name){
  ...
  for( unsigned int i = 0; i<n; i++){
    ...
    vec_size_pow3[i] = pow(vec_size[i], 3) / pow(vec_size[0], 3) * elap_time_eff[0];
  }

  //make sure the sizes match
  assert( vec_size.size() == elap_time.size() && "vector sizes must be the same.");
