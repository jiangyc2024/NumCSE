...
MatrixXd min_frob(const VectorXd &z, const VectorXd &g) {
  ...
  S << MatrixXd::Identity(n * n, n * n), C.transpose(), C, MatrixXd::Zero(n, n);
  VectorXd h(n * n + n);
  h << VectorXd::Zero(n * n), g;
  // Solve augmented system and return only head of solution
  // as a $n \times n$ matrix (discard the rest).
  return Map<const MatrixXd>(S.DECOMP().solve(h).eval().data(), n, n)
      .transpose();
}
/* SAM_LISTING_END_1 */
...
int main(int argc, char **argv) {
  ...
  /* SAM_LISTING_BEGIN_2 */
  VectorXd z = VectorXd::Random(n), g = VectorXd::Random(n);
  MatrixXd Mstar = min_frob(z, g);                  // $M^*$
  MatrixXd M = g * z.transpose() / z.squaredNorm(); // $M$
  std::cout << "Norm: " << (Mstar - M).norm() << std::endl;
  /* SAM_LISTING_END_2 */
}
