...
struct TriDiagonalMatrix {
    ...
    TriDiagonalMatrix(const Eigen::VectorXd &d,const Eigen::VectorXd &l,const Eigen::VectorXd &u)
        : d(d),
          l(l),
          u(u),
          n(d.size())
 ...
 */
Eigen::Matrix2d Givens(double rho) {
    double gamma, sigma;
    if(std::abs(rho)<1.0) {
        sigma = 2.0*rho;
...
std::tuple<double, double, double> compGivensRotation(Eigen::Vector2d a) {
    ...
    // Hint: You can use the function sign() above when determining rho from gamma and sigma.
    // START
    if (a(1) != 0.0) {
        double t;
 ...
 */
/* SAM_LISTING_BEGIN_3 */
TriDiagonalQR::TriDiagonalQR(const TriDiagonalMatrix &A)
    : n(A.n) {
    ...
    for(int k=0; k<(n-1); k++) {
        ...
        Ablock <<  B(k,0),     B(k,2),        0,
                   B(k+1,1), B(k+1,0), B(k+1,2);
        // TO DO (4-4.c): Find the k-th Givens rotation and update rho and B.
        // START
        std::tuple<double,double,double> params = compGivensRotation(Ablock.col(0));
...
Eigen::VectorXd TriDiagonalQR::applyQT(const VecType &x) const {
    assert( n == x.size() && "Input must have size equal to the number of rows in Q." );
    Eigen::VectorXd y(n);
...
Eigen::VectorXd TriDiagonalQR::solve(const VecType &b) const {
    assert( n == b.size() && "Input must have size equal to the number of rows in Q." );
    Eigen::VectorXd x(n);
...
void TriDiagonalQR::getDense(Eigen::MatrixXd& Q, Eigen::MatrixXd& R) {
    // This function is only for demonstration purposes, and is not in any way optimal.
...
unsigned int invit(const MatrixType &A, Eigen::VectorXd &x, double TOL = 1E-6, unsigned int maxit = 100) {
    x.normalize();
...
/* SAM_LISTING_BEGIN_7 */
template<>
unsigned int invit<TriDiagonalQR>(const TriDiagonalQR &A, Eigen::VectorXd &x, double TOL, unsigned int maxit) {
    x.normalize();
    Eigen::VectorXd x_old(x.size());
    int it = 0;
    do {
