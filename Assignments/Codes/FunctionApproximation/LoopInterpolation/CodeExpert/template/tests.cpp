...
TEST_SUITE("adaptiveintp") {
  TEST_CASE("std::vector<Vector2d> closedPolygonalInterpolant" * doctest::description("closedPolygonalInterpolant()")) {
  ...
  TEST_CASE("std::pair<std::vector<Vector2d>, std::vector<Vector2d>> adaptedHermiteInterpolant" *
    ...
    VectorXd x=Eigen::VectorXd::Constant(5,0.5);
    std::pair<std::vector<Eigen::Vector2d>, std::vector<Eigen::Vector2d>> sol = adaptedHermiteInterpolant(c,5,data.x,1e-3);
    std::pair<std::vector<Eigen::Vector2d>, std::vector<Eigen::Vector2d>> stud = adaptedHermiteInterpolant_TEST(c,5,data.x, 1e-3);
