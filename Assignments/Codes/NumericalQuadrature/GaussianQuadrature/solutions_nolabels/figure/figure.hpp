...
mglData make_mgldata(const Eigen::MatrixBase<Derived>& vec) {
  if (!(vec.rows() == 1 || vec.cols() == 1)) {
    std::cerr << "In function Figure::make_mgldata(): vector.cols() == 1 || "
                 "vector.rows() == 1 failed!";
    assert(vec.rows() == 1 || vec.cols() == 1);
  }
  std::vector<typename Derived::Scalar> v;
  if (vec.rows() == 1) {
    v.resize(vec.cols());
    for (int i = 0; i < vec.cols(); i++) v[i] = vec[i];
  } else {
    v.resize(vec.rows());
    for (int i = 0; i < vec.rows(); i++) v[i] = vec[i];
  }
...
class Figure {
 public:
  ...
  void grid(bool on = true, const std::string& gridType = "-",
  ...
  // Note: this tedious template expression ensures y is a Matrix and not e.g. a
  // string, which could happen
  //       as the argument is templated
  template <typename xVector, typename Matrix>
  typename std::enable_if<
      !std::is_same<
          typename std::remove_pointer<typename std::decay<Matrix>::type>::type,
          char>::value,
      MglPlot&>::type
  ...
  typename std::enable_if<
      !std::is_same<typename std::remove_pointer<
                        typename std::decay<yVector>::type>::type,
                    char>::value,
      MglPlot&>::type
  plot(const xVector& x, const yVector& y, std::string style = "");

  template <typename xVector, typename yVector, typename zVector>
  MglPlot& plot3(const xVector& x, const yVector& y, const zVector& z,
                 std::string style = "");
  ...
  void ranges(const double& xMin, const double& xMax, const double& yMin,
              const double& yMax);
...
#if FIG_HAS_EIGEN             // only enable if Eigen is available, otherwise
                              // Eigen::SparseMatrix will not be defined
  template <typename Scalar>  // sparse version
  MglPlot& spy(const Eigen::SparseMatrix<Scalar>& A,
 ...
 private:
  ...
  std::array<double, 3> aspects_;  // axis aspects, e.g. -1 used to invert axis.
                                   // see MathGL documentation
  ...
  std::vector<std::unique_ptr<MglPlot>>
      plots_;  // x, y (and z) data for the plots
  std::vector<std::pair<std::string, std::string>>
      additionalLabels_;  // manually added labels
};
...
/* bar plot for given y data * PRE : - * POST: add bar plot of [1:length(y)]-y
 * to plot queue with given style (optional) */
template <typename Matrix>
...
template <typename xVector, typename Matrix>
// the long template magic expression ensures this function is not called if
// yVector is a string (which would be allowed as it is a templated argument)
typename std::enable_if<
    !std::is_same<
        typename std::remove_pointer<typename std::decay<Matrix>::type>::type,
        char>::value,
...
Figure::bar(const xVector& x, const Matrix& y, std::string style) {
  // TODO check if x is really a vector. Problem: cant use .rows or .cols as it
  // can be a std::vector
  ...
  if (x.size() != m) {
    std::cerr << "In function Figure::bar(): Data sets must have same lengths! "
                 "Remember to save the bar-data in columns!";
  }
  ...
  // if yes: add that style to the style queue and remove it from the
  // style-deque, if no : get a new style from the style-deque
  if (style.size() == 0) {
    for (long i = 0; i < n; ++i) {
      style += styles_.get_next();
    }
  } else {
    // special characters dont matter in bar plot, remove all non-alphabetic
    // characters
    std::remove_if(style.begin(), style.end(),
  ...
  plots_.emplace_back(
      std::unique_ptr<MglBarPlot>(new MglBarPlot(xd, yd, style)));
...
typename std::enable_if<
    !std::is_same<
        typename std::remove_pointer<typename std::decay<yVector>::type>::type,
        char>::value,
    MglPlot&>::type
...
template <typename xVector, typename yVector, typename zVector>
MglPlot& Figure::plot3(const xVector& x, const yVector& y, const zVector& z,
                       std::string style) {
  ...
  plots_.emplace_back(
      std::unique_ptr<MglPlot3d>(new MglPlot3d(xd, yd, zd, style)));
  return *plots_.back().get();
}
...
MglPlot& Figure::spy(const Matrix& A, const std::string& style) {
  ...
  mglData xd(x.data(), x.size()), yd(y.data(), y.size());
  ...
  plots_.emplace_back(
      std::unique_ptr<MglSpy>(new MglSpy(xd, yd, style + radius)));
  return *plots_.back().get();
}

#if FIG_HAS_EIGEN
template <typename Scalar>
MglPlot& Figure::spy(const Eigen::SparseMatrix<Scalar>& A,
  ...
  ranges_ = std::array<double, 4>{0, (static_cast<double>(A.cols() + 1)), 0,
                                  (static_cast<double>(A.rows() + 1))};
  // iterate over nonzero entries, using method suggested in Eigen::Sparse
  // documentation
...
template <typename Scalar, typename xVector, typename yVector>
MglPlot& Figure::triplot(
    const Eigen::Matrix<Scalar, -1, -1, Eigen::RowMajor>& T, const xVector& x,
    const yVector& y, std::string style) {
  Eigen::Matrix<double, -1, -1, Eigen::RowMajor> T_double =
      T.template cast<double>();

  mglData Td(T_double.rows(), T_double.cols(), T_double.data()),
      xd(x.data(), x.size()), yd(y.data(), y.size());
  ...
  // check if a style is given,
  // if yes: add that style to the style queue and remove it from the
  // style-deque, if no : get a new style from the style-deque Note: '#' is to
  // draw lines and not a fully colored plot,
  ...
  // put the x-y data in the plot queue
  plots_.emplace_back(std::unique_ptr<MglTriPlot>(
      new MglTriPlot(Td, xd, yd, style, draw_numbers)));
  return *plots_.back().get();
}

template <typename Scalar, typename xVector, typename yVector>
MglPlot& Figure::triplot(
    const Eigen::Matrix<Scalar, -1, -1, Eigen::ColMajor>& T, const xVector& x,
    const yVector& y, std::string style) {
