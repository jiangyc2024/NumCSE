 ...
 * A = {0, 1}, B = {a, b, bc}
 * => C = {0a, 1a, 0b, 1b, 0c, 1c}     */
static void crossjoin(const std::deque<std::string>& A,
...
void MglStyle::get_new(std::deque<std::string>& new_deque) {
  std::deque<std::string> colors = {"b", "r", "g", "c", "m", "y",
                                    "G", "p", "q", "k", "n"},
                          linetypes = {"-", ":", ";", "|", "j", "i", "="};
...
MglStyle::MglStyle() { get_new(styles_); }

/* create new styles_ deque and remove 'already_used' * PRE : - * POST: new
 * styles get appended to style_ deque and 'already_used' gets eliminated */
...
/* sort string to default layout: <solid?><color><styleoption><linewidth>, as in
 * "b:0", "r-1", " r*"                 * if parameters are missing choose
 * default: solid -> "" (true), color -> 'b' , styleoption -> '-', linewidth ->
 * '0' * PRE : style string containing valid characters only (see below or
 * MathGL documentation)                           * POST: string gets
 * re-arranged to a certain order: (solid)(color)(linestyle)(linewidth) */
static std::string normalized(const std::string& s) {
  // get parameters
  std::string solid = "", color = "b", styleoption = "", linewidth = "0";

  auto it = s.begin();

  const std::string
      valid_colors =
          "bgrhwBGRHWcmypCMYkPlenuqLENUQ",  // colors from MathGL documentation
      valid_styles =
          ".+x*sdo^v<>#-|l;=ji: AVKIDSOX_S";  // . to # -- markers, - to : --
                                              // linetypes, rest -- arrowstyles
  ...
  while (it != s.end()) {
    ...
    else if (std::find(valid_colors.begin(), valid_colors.end(), *it) !=
             valid_colors.end()) {
      color = *it;
      ++it;
      continue;
    ...
    // check stylesoptions
    else if (std::find(valid_styles.begin(), valid_styles.end(), *it) !=
             valid_styles.end()) {
...
void MglStyle::eliminate(const std::string& already_used) {
  ...
  if (already_used_normalized[2] != '0' && already_used_normalized[2] != '1') {
    return;  // if the width is != 0 we dont even have to search for this style
             // in our deque
  } else {
