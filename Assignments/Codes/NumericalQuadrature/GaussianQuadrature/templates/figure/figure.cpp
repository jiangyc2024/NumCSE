...
double minPositive(const mglData& d) {
  ...
  for (long i = 0; i < d.GetNx(); ++i) {
    if (d.a[i] > 0) {
      // if the data point is positive, check if it is smaller than the current
      // minimum
      result = std::min(result, d.a[i]);
    } else {
      // if the data point is not positive it will not appear on the plot ->
      // print warning
      print_warning = true;
    }
  }
  ...
  if (print_warning) {
    std::cerr << "* Figure - Warning * non-positive values of data will not "
                 "appear on plot. \n";
  }
...
/* constructor: set default style * PRE : pointer to mglGraph will not cease to
 * exist until operations are performed on this Figure *
...
Figure::Figure()
    : autoRanges_(true),
      ...
      has_3d_(false),
      legend_(false),
      figHeight_(-1),  // set to -1: later we will check if they have been
                       // changed manually, -1 means no
      figWidth_(-1),   //            any other value will mean that they've been
                       //            changed
      plotHeight_(800),  // quadratic plot, window size depends on wheter there
                         // are labels or not!
      plotWidth_(800),
      leftMargin_(-1),
      topMargin_(-1),
      ...
      ranges_({std::numeric_limits<double>::max(),
               std::numeric_limits<double>::lowest(),
               std::numeric_limits<double>::max(),
               std::numeric_limits<double>::lowest()}),
      zranges_({std::numeric_limits<double>::max(),
                std::numeric_limits<double>::lowest()}),
      styles_(MglStyle()) {}
...
void Figure::addlabel(const std::string& label, const std::string& style) {
  additionalLabels_.push_back(
      std::pair<std::string, std::string>(label, style));
}

/* change grid settings * PRE : - * POST: No grid if on is false, grid style is
 * gridType, grid color is gridCol. * For those arguments which are not given
 * default settings are used.     */
void Figure::grid(bool on, const std::string& gridType,
                  const std::string& gridCol) {
  if (on) {
    grid_ = true;
  }
  // although in the constructor grid_ is set to false this explicit setting to
  // false is necessary as otherwise the grid couldn't be turned off once
  // activated
  else {
    grid_ = false;
  }
...
void Figure::legend(const double& xPos, const double& yPos) {
  // print a warning if the user has given a position which will probably not
  // appear on the plot
  if (std::abs(xPos) > 2 || std::abs(yPos) > 2) {
    std::cerr << "* Figure - Warning * Legend may be out of the graphic due to "
                 "large xPos or yPos\n";
  }
...
/* plot a function given by a string * PRE : proper format of the input, e.g.:
 * "3*x^2 + exp(x)", see documentation for more details * POST: plot the
 * function in given style */
...
void Figure::ranges(const double& xMin, const double& xMax, const double& yMin,
                    const double& yMax) {
  // checking if the input is valid
  if (xMin > xMax || yMin > yMax) {
    std::cerr << "In function Figure::ranges(): xMin must be smaller than xMax "
                 "and yMin smaller than yMax!";
  }
...
/* change ranges of plot * PRE : - * POST: set ranges in such a way that all
 * data is displayed                         * NOTE: need the argument
 * vertMargin to be able to set it to 0 when plotting in 3d  */
void Figure::setRanges(const mglData& xd, const mglData& yd,
  ...
  // if yes: if the minimal value is <= 0 set xMin (or yMin) to the smallest
  // positive number in the data
  //         and if the maximal value is <= 0 no data will appear -> error
  //         message!
  // if no:  leave the old value
  if (xFunc_ == "lg(x)") {
    if (xMax <= 0) {
      std::cerr << "In function Figure::setRanges() : Invalid ranges for "
                   "logscaled plot - maximal x-value must be greater than 0.";
    }
    yMin = minPositive(xd);
  }
  if (yFunc_ == "lg(y)") {
    if (yMax <= 0) {
      std::cerr << "In function Figure::setRanges() : Invalid ranges for "
                   "logscaled plot - maximal y-value must be greater than 0.";
    }
  ...
  ranges_[2] =
      std::min(yMin - yTot * vertMargin,
               ranges_[2]);  // adding a slight margin in linear plots on bottom
...
void Figure::setRanges(const mglData& xd, const mglData& yd,
  ...
  // if yes: if the minimal value is <= 0 set zMin to the smallest positive
  // number in the data
  //         and if the maximal value is <= 0 no data will appear -> error
  //         message!
  // if no:  leave the old value
  if (zFunc_ == "lg(z)") {
    if (zMax <= 0) {
      std::cerr << "In function Figure::setRanges() : Invalid ranges for "
                   "logscaled plot - maximal z-value must be greater than 0.";
    }
    zMin = minPositive(zd);
  }
...
/* (un-)set logscaling * PRE : - * POST: linear, semilogx, semilogy or loglog
 * scale according to bools logx and logy */
void Figure::setlog(bool logx, bool logy, bool logz) {
  // if logi is true set iFunc_ to "lg(i)", which will later be used to
  // initialize the coordinate curvature (i=x,y,z)
  if (logx) {
    xFunc_ = "lg(x)";
  }
  if (logy) {
...
void Figure::title(const std::string& text) { title_ = "@{" + text + "}"; }
...
void Figure::save(const std::string& file) {
  ...
  if (figWidth_ == -1 || figHeight_ == -1 || topMargin_ == -1 ||
      leftMargin_ == -1) {
  ...
  // Shorten tick marks (factor 0.01) and make subticks so small that they do
  // not appear (factor 1000)
  ...
  if (has_3d_) {
    ...
    gr_.SetRanges(ranges_[0], ranges_[1], ranges_[2], ranges_[3], zranges_[0],
                  zranges_[1]);
    gr_.Rotate(60, 30);
  } else {
    gr_.SubPlot(1, 1, 0, "#");
  ...
  if (!has_3d_) {
    gr_.InPlot(
        double(leftMargin_) / figWidth_,               // margin from left
        double(leftMargin_ + plotWidth_) / figWidth_,  // how far to the right
        double(figHeight_ - plotHeight_ - topMargin_) /
            figHeight_,                                 // how far to the bottom
    ...
    // Note: This *has* to be called after SubPlot and InPlot, otherwise the
    // axis labels will be in 1,1,1 manner
    gr_.Aspect(aspects_[0], aspects_[1], aspects_[2]);
  }

  // Set label - before setting curvilinear because MathGL is vulnerable to
  // errors otherwise
...
#define NAN (*(float*)(mgl_nan + 1))
  ...
  if (legend_) {
    if (!has_3d_) {
      // scale legend input according to figHeight, figWidth, plotHeight,
      // plotWidth, etc.
