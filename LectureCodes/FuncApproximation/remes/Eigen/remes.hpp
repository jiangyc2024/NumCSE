...
//      c will be saved to save the coefficients of the interpolant in monomial basis
...
void remez(const Function& f, const Derivative& df, const double a, const double b, const unsigned d, const double tol, VectorXd& c) {
  ...
  for (unsigned k = 0; k < maxit; ++k) {
    ...
    VectorXd cd(d);        // to compute monomial coefficients of derivative \Blue{$q'$}
    ...
    // \com{Secant method} to determin zeros of derivative of approximation error
    ...
    del << polyval(c.head(d + 1), a*VectorXd::Ones(1)) - ftab(0)*VectorXd::Ones(1),
           polyval(c.head(d + 1), xe) - fxe,
           polyval(c.head(d + 1), b*VectorXd::Ones(1)) - ftab(ftab.size() - 1)*VectorXd::Ones(1);
