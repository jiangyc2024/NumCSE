...
VectorXd trigpolyval(const VectorXd& t, const VectorXd& y, const VectorXd& x) {
  ...
  if (N % 2 == 0) {
    throw std::runtime_error("Number of points must be odd!");
  }
  const auto n = static_cast<double>(N - 1)/2;
  const std::complex<double> M_I(0,1); // imaginary unit
  // interpolation nodes and evalutation points on unit circle
  const VectorXcd tc = ( 2*M_PI*M_I*t ).array().exp().matrix();
  const VectorXcd xc = ( 2*M_PI*M_I*x ).array().exp().matrix();
  // Rescaled values, according to \Blue{$q(t) = e^{-2\pi int}\cdot p(e^{2\pi it})$}, see \eqref{eq:trigpcomp}
  const VectorXcd z = ((2*n*M_PI*M_I*t).array().exp() * y.array()).matrix();
  ...
  const BarycPolyInterp<comp> Interpol(tc);
  auto p = Interpol.eval<VectorXcd>(z,xc);
...
void trigpolyval(const VectorXd& t, const VectorXd& y, const VectorXd& x, VectorXd& q) {
  ...
  const VectorXcd xc = ( 2*M_PI*i*x ).array().exp().matrix();
  ...
  VectorXcd p; intpolyval::intpolyval(tc, z, xc, p);
  // Undo the scaling, see \eqref{eq:scale} 
