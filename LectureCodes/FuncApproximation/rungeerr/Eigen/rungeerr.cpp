...
int main() {
  ...
  // 1000 sampling points for approximate maximum norm
  const VectorXd x = VectorXd::LinSpaced(1000, -5, 5);
  // Sample function
  const VectorXd fx = x.unaryExpr(f); // evaluate f at x
  ...
  for (int d = 1; d <= 20; ++d) {
    ...
    // Compute interpolating polynomial in monomial representation
    const VectorXd p = polyfit(t, ft, d);
    // Evaluate polynomial interpolant in sampling points
