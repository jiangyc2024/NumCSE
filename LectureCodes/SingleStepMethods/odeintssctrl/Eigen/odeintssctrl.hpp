...
odeintssctrl(DiscEvolOp &&Psilow, unsigned int p, DiscEvolOp &&Psihigh,
  ...
  std::vector<std::pair<double,State>> states; // vector $\cob{\left(t_k,\Vy_k\right)_k}$
  ...
  while ((states.back().first < T) && (h >= hmin)) { // \Label[line]{ssctrl:2}
    ...
    double tol = max(reltol*norm(y), abstol); // effective tolerance \Label[line]{ssctrl:6a}
    ...
    if (est < tol)  // step \Magenta{accepted} \Label[line]{ssctrl:7}\Label[line]{ssctrl:6}
      states.push_back({t = t+min(T-t,h),y = yh}); // store next approximate state 
  }
  if (h < hmin) {
