...
//! @param[in] T Matrix $M \times 3$ containing the vertex numbers of each of the $M$ triangles of a triangular mesh
//! @param[out] E Matrix $\sharp \mathcal{E} \times 2$ whose rows correspond to the edges of the mesh and give the indices (corresponding to T) of the endpoints of the edges
//! @param[out] Eb Matrix $\sharp \mathcal{\Gamma} \times 2$ whose rows correspond to the boundary edges of the mesh and give the indices (corresponding to T) of the endpoints of the edges
void processmesh(const Eigen::MatrixXi &T, Eigen::MatrixXi & E, Eigen::MatrixXi &Eb){
	...
	for(int k = 0; k < M; ++k){
		...
		*/
		for(int i = 0; i < 2; ++i){
			for(int j = i+1; j < 3; ++j){
				if(T(k,i) < T(k,j))	// insert combinations sorted wrt. to node numbering (ascending)
	...
	Eb = Eigen::Matrix<int, -1, -1,Eigen::RowMajor>::Map(eb.data(), eb.size()/2, 2);	
}
/* SAM_LISTING_END_0 */
...
//! @param[in] T Matrix $M \times 3$ containing the vertex numbers of each of the M triangles of a triangular mesh
//! @param[in] E Matrix $\sharp \mathcal{E} \times 2$ whose rows correspond to the edges of the mesh and give the indices (corresponding to T) of the endpoints of the edges
//! @param[out] ET Matrix $N \times 3$ whose rows contain the index numbers of the edges of each triangle
...
/* SAM_LISTING_BEGIN_2 */
//! @brief \eigen function extracting the triangle edge mapping
//! @param[in] x Vector of dim $N$ containing the x-coordinates of nodes of the mesh
//! @param[in] y Vector of dim $N$ containing the y-coordinates of nodes of the mesh
//! @param[in] T Matrix $M \times 3$ containing the vertex numbers of each of the M triangles of a triangular mesh
//! @param[out] x_ref Vector of dim $N$ containing the x-coordinates of nodes of the refined mesh
//! @param[out] y_ref Vector of dim $N$ containing the y-coordinates of nodes of the refined mesh
//! @param[out] T_ref Matrix $M \times 3$ containing the vertex numbers of each of the M triangles of a triangular refined mesh
...
/* SAM_LISTING_BEGIN_3 */
//! @brief \eigen function for smoothing a planar triangular mesh by moving all vertices to the barycenter of their neighboring nodes.
//! @param[in] x Vector of dim $N$ containing the x-coordinates of nodes of the mesh
//! @param[in] y Vector of dim $N$ containing the y-coordinates of nodes of the mesh
//! @param[in] T Matrix $M \times 3$ containing the vertex numbers of each of the M triangles of a triangular mesh
//! @param[out] xs Vector of dim $N$ containing the x-coordinates at the barycenter of their neighboring nodes
//! @param[out] ys Vector of dim $N$ containing the y-coordinates at the barycenter of their neighboring nodes
