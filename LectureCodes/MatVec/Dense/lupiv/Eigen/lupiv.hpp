...
void lupiv(MatrixXd &A){//insitu
	...
	for(int k = 0; k < n-1; ++k){
		int j = -1; //j = pivot row index  
		// p = relativly largest pivot
		const double p = ( A.col(k).tail(n-k).cwiseAbs().cwiseQuotient( A.block(k,k,n-k,n-k).cwiseAbs().rowwise().maxCoeff() ) ).maxCoeff(&j); // \label{gsrp:1}
		...
		const  VectorXd fac = A.col(k).tail(n-k-1) / A(k,k);// \label{gsrp:f}
