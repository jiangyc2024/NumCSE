...
void gepiv(const MatrixXd &A, const VectorXd& b, VectorXd& x){
	...
	for(int k = 0; k < n-1; ++k){
		...
		p = ( Ab.col(k).tail(n-k).cwiseAbs().cwiseQuotient(
			Ab.block(k,k,n-k,n-k).cwiseAbs().rowwise().maxCoeff() ) ).maxCoeff(&j);// \label{lup:1}
		...
		Ab.bottomRightCorner(n-k-1,n-k) -= Ab.col(k).tail(n-k-1)
								* Ab.row(k).tail(n-k) / Ab(k,k);// \label{lup:4} 
	}
