...
int main (int argc, char * const argv[]) {
	...
	for (int r=0;r<rep;++r)
	{
	watch.start(); C=A.standardMultiply(B); T0=std::min(T0,watch.getTime()); watch.reset(); 
	watch.start(); D=A.dotMultiply(B);      T1=std::min(T1,watch.getTime()); watch.reset();
	watch.start(); E=A.gemvMultiply(B);     T2=std::min(T2,watch.getTime()); watch.reset();
	watch.start(); F=A.gemmMultiply(B);     T3=std::min(T3,watch.getTime()); watch.reset();
	}
