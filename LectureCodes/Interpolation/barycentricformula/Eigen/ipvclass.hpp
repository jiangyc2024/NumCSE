...
/* SAM_LISTING_BEGIN_0 */
template <typename NODESCALAR = double> class BarycPolyInterp {
private:
...
BarycPolyInterp<NODESCALAR>::BarycPolyInterp(const nodeVec_t &_t)
    : n(_t.size()), t(_t), lambda(n) {
  init_lambda();
}
...
RESVEC BarycPolyInterp<NODESCALAR>::eval(const DATAVEC &y,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (z.cwiseAbs().minCoeff(&k) <
        tref * std::abs(std::numeric_limits<NODESCALAR>::epsilon())) {
...
RESVEC BarycPolyInterp<NODESCALAR>::eval
  ...
  RESVEC p(N); // Ouput vector
  // Compute quotient of weighted sums  of \Blue{$\frac{\lambda_i}{t - t_i}$},
effort \Blue{$O(n)$} for (unsigned i = 0; i < N; ++i) { nodeVec_t z =
(x[i]*nodeVec_t::Ones(n) - t);

    // check if we want to evaluate at a node <-> avoid division by zero
    NODESCALAR* ptr = std::find(z.data(), z.data() + n, NODESCALAR(0)); //
\Label{pv:1} if (ptr != z.data() + n) { // zero component in vector z? p[i] =
y(ptr - z.data()); // If yes, we have accidentally hit a node
    }
    else {
      const nodeVec_t mu = lambda.cwiseQuotient(z);
      p[i] = (mu.cwiseProduct(y)).sum()/mu.sum();
...
void BarycPolyInterp<NODESCALAR>::init_lambda(void) {
  ...
  for (unsigned k = 0; k < n; ++k) {
    ...
    lambda(k) =
        1. / ((t(k) * nodeVec_t::Ones(k) - t.head(k)).prod() *
