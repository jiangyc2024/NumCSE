...
double adaptquad(Function& f, VectorXd& M,double rtol,double atol) {
  ...
  // distance of quadature nodes \Label[line]{aq:1}
  const VectorXd h = M.tail(n-1)-M.head(n-1); 
  const VectorXd mp = 0.5*(M.head(n-1)+M.tail(n-1)); // midpoints \Label[line]{aq:2}
  ...
  if ( err_tot > rtol*std::abs(I) && err_tot > atol ) { // \Label[line]{aq:term}
    ...
    for (unsigned i = 0; i < est_loc.size(); ++i) {
      ...
      if (est_loc(i) > 0.9/static_cast<double>(n-1)*err_tot) { 
